...
export default function Reportes() {
  ...
  async function loadReportes() {
    const r = await api.get(`/reportes/ventas?desde=${desde}&hasta=${hasta}`);
    setVentas(r || []);
  }
  ...
  function imprimirArqueo() {
    ...
    w.document.write(`
      ...
      <p><b>Total ventas: Bs. ${Number(arqueoData.ventas?.total_ventas || 0).toFixed(2)}</b></p>
              ...
              <>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1rem", marginBottom: "1.5rem" }}>
                  <div style={{ background: "#f3e5f5", borderRadius: "8px", padding: "1rem", textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: "1.8rem", fontWeight: "bold", color: "#9C27B0" }}>{ventas.reduce((s, v) => s + Number(v.total_ventas), 0)}</div>
                  </div>
